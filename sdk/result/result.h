...
public:
    ...
    template<typename U>
    explicit constexpr Err(const U& val) : m_value(val) {}
    template<typename U>
    explicit constexpr Err(U&& val) : m_value(std::move(val)) {}
...
namespace std {
template <typename T, typename E>
struct hash<result_sdk::Result<T, E>> {
    std::size_t operator()(const result_sdk::Result<T, E>& result) {
        if(result.is_ok()) {
